...
public class DetailVerifikasiActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void setData(MonitoringModel monitoringModel) {
        ...
        if (monitoringModel.getLAST_STATUS() == 4){
            speedDialView.addActionItem(
                    new SpeedDialActionItem.Builder(R.id.action_verified_closed, R.drawable.ic_verified)
                            .setFabBackgroundColor(getResources().getColor(R.color.blue_btn_bg_color))
                            .setFabSize(FloatingActionButton.SIZE_AUTO)
                            .setLabel(getResources().getString(R.string.actionVerifiedClosed)).create());
            ...
            public boolean onActionSelected(SpeedDialActionItem actionItem) {
                switch (actionItem.getId()) {
                    case R.id.action_detail_status:
                        Common.showDetailStatusMonitoring(DetailVerifikasiActivity.this);
                        speedDialView.close();
                        break;
                    case R.id.action_verified_closed:
     ...
     */
    private void showDialogKomentar(boolean verifiedAndClosed, boolean revisi) {
        speedDialView.close();
                ...
                .setConfirmClickListener(sweetAlertDialogMain -> {
                    ...
                    if (verifiedAndClosed) {
                        new SweetAlertDialog(this, SweetAlertDialog.CUSTOM_IMAGE_TYPE)
                                ...
                                .setConfirmText("Verified and Closed")
                                .setConfirmClickListener(sweetAlertDialog -> {
                                    verifiedAndClosed(komentar, sweetAlertDialogMain, sweetAlertDialog);
                                })
                                .show();
                    }
...
//            });
        ...
        mService.updateStatusMonitoring(ID_MON, LAST_STATUS, komentar).enqueue(new Callback<ResponseModel>() {
            ...
            public void onResponse(Call<ResponseModel> call, Response<ResponseModel> response) {
                if (response.isSuccessful()){
                    ...
                    Common.insertLogAction(DetailVerifikasiActivity.this, aksi, "");
                }
            }
     ...
     * Verified and Closed
     */
    private void verifiedAndClosed(String komentar, SweetAlertDialog sweetAlertDialogMain, SweetAlertDialog sweetAlertDialog) {
        String tgl_approved = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(new Date());
        String ID_MON = Common.selectedMonitoring.getID_MON();
        int LAST_STATUS = 5;  /** Verified*/
        ...
        mService.updateStatusMon5(ID_MON, tgl_approved, komentar, LAST_STATUS).enqueue(new Callback<ResponseModel>() {
            @Override
            public void onResponse(Call<ResponseModel> call, Response<ResponseModel> response) {
                if (response.isSuccessful()) {
                    sweetAlertDialog.setTitleText("Success!")
                            .showCancelButton(false)
                            .setContentText(getString(R.string.contentVerifiedAndClosed))
                            .setConfirmText("Close")
                            .setConfirmClickListener(sweetAlertDialog1 -> {
                                sweetAlertDialogMain.dismissWithAnimation();
                                sweetAlertDialog1.dismissWithAnimation();
                    ...
                    /** Insert Log */
                    String aksi = "Verified Task - " + Common.selectedMonitoring.getID_MON();
                    Common.insertLogAction(DetailVerifikasiActivity.this, aksi, "");
                }
            }
